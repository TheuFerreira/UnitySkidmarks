    ...
    public class SkidmarksController : MonoBehaviour
    {
        ...
        public int AddSkidMark(Vector3 pos, Vector3 normal, Color32 colour, int lastIndex)
        {
            ...
            if (lastSection != null)
            {
                ...
                curSection.Posl = curSection.Pos + 0.5f * MARK_WIDTH * xDirection;
                curSection.Posr = curSection.Pos - 0.5f * MARK_WIDTH * xDirection;
                curSection.Tangent = new Vector4(xDirection.x, xDirection.y, xDirection.z, 1);

                if (lastSection.LastIndex == -1)
                {
                    lastSection.Tangent = curSection.Tangent;
                    lastSection.Posl = curSection.Pos + 0.5f * MARK_WIDTH * xDirection;
                    lastSection.Posr = curSection.Pos - 0.5f * MARK_WIDTH * xDirection;
                }
            }
